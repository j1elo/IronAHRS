...
void setup() {
  ...
  // Load font
  font = loadFont("../common/Univers-66.vlw");
  textFont(font);
...
void setupRazor() {
  ...
  serial.write("qob");  // Turn on binary output
  serial.write("qo1");  // Turn on continuous streaming output
  serial.write("qoe0"); // Disable error message output
  
  // Synch with Razor
  serial.clear();  // Clear input buffer up to here
  serial.write("qs00");  // Request synch token
}
...
void keyPressed() {
  switch (key) {
    ...
    case 'a':  // Align screen with Razor
      yawOffset = yaw;
      break;
    case 'r':  // Request one single yaw/pitch/roll frame from Razor (use when continuous streaming is off)
      serial.write("qr");
      break;
  }
}
